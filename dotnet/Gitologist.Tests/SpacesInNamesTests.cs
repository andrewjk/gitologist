...
public class SpacesInNamesTests
{
    ...
    [TestMethod]
    public async Task ShouldAddFileWithLeadingSpaceInName()
    {
        await Init.InitRepo(_testDir);
        await File.WriteAllTextAsync(Path.Combine(_testDir, " leading.txt"), "content");

        await Add.AddFiles(_testDir, new[] { " leading.txt" });
    ...
    [TestMethod]
    public async Task ShouldUpdateStatusAfterRestoringFileWithSpace()
    {
        await Init.InitRepo(_testDir);
        await File.WriteAllTextAsync(Path.Combine(_testDir, "test file.txt"), "original");
        await Add.AddFiles(_testDir, new[] { "test file.txt" });
        ...
        var result = await Status.GetStatus(_testDir);
        Assert.IsTrue(result.Modified.Contains("test file.txt"));
