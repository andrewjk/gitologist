...
// 2. Copy Swift files to sibling gitologist-swift
const swiftSrc = path.join(rootDir, "swift");
const swiftDest = path.join(rootDir, "..", "gitologist-swift");
console.log(`Copying ${swiftSrc} to ${swiftDest}`);
await clearDest(swiftDest);
await copyDirectory(swiftSrc, swiftDest, [".build"]);

// 3. Copy Zig files to sibling gitologist-zig
const zigSrc = path.join(rootDir, "zig");
const zigDest = path.join(rootDir, "..", "gitologist-zig");
...
async function updateCSharpVersion(version: string) {
	const csprojPath = path.join(rootDir, "dotnet", "Gitologist", "Gitologist.csproj");
